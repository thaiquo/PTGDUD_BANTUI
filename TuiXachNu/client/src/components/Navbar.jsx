...
import { ProductContext } from "../context/ProductProvider"; // lấy currentUser từ context
...
const Navbar = () => {
  ...
  const { currentUser } = useContext(ProductContext); // dùng context
  ...
  const getLinkClass = (link) => {
    return location.pathname === link ? "text-red-500 font-bold" : "";
  };
        ...
        <ul className="flex space-x-8 font-medium text-gray-700">
          {menu.map((item, index) => (
            <li key={index} className="relative" onMouseEnter={() => handleMouseEnter(index)} onMouseLeave={handleMouseLeave}>
              <Link to={item.link || "#"} className={`${getLinkClass(item.link)} ${activeIndex === index ? "text-red-500" : ""}`}>
                {item.title}
              </Link>
